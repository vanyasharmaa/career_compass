...
export async function POST(req: Request) {
  try {
    ...
    // ⭐ Use the model that works - gemma-3-27b-it
    const model = genAI.getGenerativeModel({ model: "gemma-3-27b-it" })

    let historyContext = ""
    if (userRatings && userRatings.length > 0) {
      historyContext = `\n\nUser's past ratings:\n${userRatings.map((r: any) => 
        `- Rated "${r.eventTitle}" ${r.rating}/5 stars`
      ).join('\n')}`
    }
...
- Event Rating: ${event.averageRating || 'No rating'}/5
- Club: ${event.club}
`).join('\n---\n')}
...
TASK: Rank ALL ${events.length} events from MOST to LEAST relevant for this student.

Return ONLY a JSON array of event IDs in ranked order: ["event-id-1", "event-id-2", "event-id-3"]

Do not include any explanation, just the JSON array.`

    console.log('🤖 Calling Gemini API with gemma-3-27b-it...')
    const result = await model.generateContent(prompt)
    const text = result.response.text().trim()

    console.log('📝 Gemini response:', text)

    // TEMP: force invalid JSON to trigger career+ratings fallback (uncomment to activate fallback)
    //const cleanText = "this is not json"

    
    // Clean response (comment to activate fallback)
    let cleanText = text
   ...
   //
    ...
    try {
      ...
      return NextResponse.json({
        rankedEvents: events.sort((a: any, b: any) => 
